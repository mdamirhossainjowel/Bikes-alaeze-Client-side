...
const CheckoutForm = (props) => {
  ...
  const {paymentIntent,error:intenerror}=await stripe.confirmCardPayment(clientSecret, {
    payment_method: {
      card: card,
      billing_details: {
        name: Name,
  ...
  if(intenerror){
    ...
    return (
        ...
        <CardElement />
        <button type="submit" className='btn btn-xs mt-3' disabled={!stripe||!clientSecret}>
          Pay
        </button>
        {
